...
public class GetLocationDurationData {
    ...
    public static void fromSE41(String assignmentId) {
        try {
...
//                    String assignmentId = returnElem.getElementsByTagName("assignmentReference").item(0).getTextContent();
    ...
    private static void fromSE19(String inventory) {
        try {
            String soapEndpointUrl = "http://dtraflocorh1134:7150/logistics-query/edm/movements"; // replace with actual service endpoint
            ...
            connection.setRequestMethod("POST");
            connection.setRequestProperty("Content-Type", "text/xml;charset=UTF-8");
            connection.setRequestProperty("user", "S-1-5-21-2000478354-838170752-1801674531-535965"); // <-- optional HTTP header
//            connection.setRequestProperty("SOAPAction", soapAction);
            connection.setDoOutput(true);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                for (int i = 0; i < invNodes.getLength(); i++) {
                    ...
                    // Inventory Id
                    String inventoryId = returnElem.getElementsByTagName("inventoryId").item(0).getTextContent();
                    System.out.println("SE19");

                    // Outer locationDurations (container)
                    NodeList outerLocDurations = returnElem.getElementsByTagName("locationDurations");
                    for (int j = 0; j < outerLocDurations.getLength(); j++) {
                        Element outerLocElem = (Element) outerLocDurations.item(j);
                        NodeList innerLocDurations = outerLocElem.getElementsByTagName("locationDurations");
